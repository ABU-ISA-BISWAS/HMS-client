...
const AddLaboratorist = () => {
    ...
    const onSubmit = async data => {
        ...
        .then(result=>{
           if(result.success){
               ...
               fetch(' http://localhost:5000/laboratorist',{
                   method:'POST',
                   headers:{
                       'content-type':'application/json',
                       authorization:`Bearer ${localStorage.getItem('accessToken')}`
                   },
                   body: JSON.stringify(laboratorist)
               ...
               .then(inserted=>{
                if(inserted.insertedId){
                    ...
                    toast.error('Failed to add the Laboratorist');
                }
               })
           }
        })
    ...
    return (
        <div className=''>
            <Link className='bg-yellow-500 border-2 border-yellow-500 rounded-lg p-2 mb-4' to="/dashboard/laboratorist">  ⇚</Link>
        <h2 className='ml-1 font-serif font-bold text-lg text-sky-800 mt-5'>Add a new laboratorist</h2>
